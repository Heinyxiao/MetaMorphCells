...
# Import imputed data
imputed_data <- read.csv("~/Library/CloudStorage/GoogleDrive-heinyxiao@gmail.com/My Drive/Lab/Projects/Dedifferentiation/Data/GSE165897_EOC_expression_matrix.csv", row.names = 1)
...
```{r}
# Load scTour precessed expression matrix and metadata
expression_data <- read.csv('~/Library/CloudStorage/GoogleDrive-heinyxiao@gmail.com/My Drive/Lab/Projects/Dedifferentiation/Data/GSE165897_EOC_expression_matrix.csv', row.names = 1)
expression_data <- t(expression_data)
head(expression_data[,1:8],8)
metadata <- read.csv('~/Library/CloudStorage/GoogleDrive-heinyxiao@gmail.com/My Drive/Lab/Projects/Dedifferentiation/Data/GSE165897_EOC_metadata.csv', row.names = 1)

# Create a new Seurat object
new_seurat_obj <- CreateSeuratObject(counts = expression_data)
...
annotation <- data.frame(group = new_seurat_obj@meta.data$treatment_phase) %>% set_rownames(., colnames(new_seurat_obj))
...
data_to_plot <- data.frame(
  cluster = cytotrace2_result$cell_subtype, 
...
VlnPlot(cytotrace2_result, features = "FZD4", pt.size = .1, group.by = "CytoTRACE2_Potency")
VlnPlot(cytotrace2_result, features = "FZD8", pt.size = .1, group.by = "CytoTRACE2_Potency")
...
# Extrace CytoTRACE2 metadata
CytoTRACE2_Potency <- cytotrace2_result@meta.data[, "CytoTRACE2_Potency", drop = FALSE]
CytoTRACE2_Score <- cytotrace2_result@meta.data[, "CytoTRACE2_Score", drop = FALSE]

# Combine expression data with metadata
expression_data <- cbind(expression_data, CytoTRACE2_Potency, CytoTRACE2_Score)
...
cor(expression_data$ROR2, expression_data$FZD8)
```


```{r}
# Draw correlation graphs
ggplot(expression_data, aes(x = ROR2, y = FZD4, colour = CytoTRACE2_Potency)) + 
  geom_point(size = 1, shape = 21, fill = "white") +
  stat_cor(p.accuracy = 0.001, r.accuracy = 0.01) + 
  labs(y = "FZD8", x = "ROR2") + 
  geom_smooth(method = "lm", color = "red", se = TRUE) + 
  theme_classic()
ggplot(expression_data, aes(x = ROR2, y = FZD8, colour = CytoTRACE2_Potency)) +
  geom_point(size = 1, shape = 21, fill = "white") +  # Apply color mapping to points
  stat_cor(p.accuracy = 0.001, r.accuracy = 0.01, aes(color = NULL)) +  # Ignore color for stat_cor
  labs(y = "FZD8", x = "ROR2") +
  geom_smooth(method = "lm", se = TRUE, color = "red", aes(color = NULL)) +  # Red regression line
  theme_classic() 
...
p <- ggplot(df, aes(x = ROR2, y = FZD8, color = CytoTRACE_Potency)) +
  geom_point(alpha = 0.6) +  # Adjust point transparency with alpha
  scale_color_viridis_c() +  # Color scale for CytoTRACE Potency
  labs(
    title = "Correlation between ROR2 and FZD8 by CytoTRACE Potency",
    x = "Expression of ROR2",
    y = "Expression of FZD8",
    color = "CytoTRACE Potency"
  ) +
...
```{r}
saveRDS(cytotrace2_result, file = "~/Library/CloudStorage/GoogleDrive-heinyxiao@gmail.com/My Drive/Lab/Projects/Dedifferentiation/Data/GSE165897_EOC_with_ptime_cytotrace.rds")

# Extract metadata
new_metadata <- cytotrace2_result@meta.data
write.csv(new_metadata, "~/Library/CloudStorage/GoogleDrive-heinyxiao@gmail.com/My Drive/Lab/Projects/Dedifferentiation/Data/GSE165897_EOC_new_metadata.csv", row.names = TRUE)
```
